...
HWND hGlobalDialog;
    ...
    void GetModelessDialog(HWND *phWnd);
	BOOL bCenterWindow ( HWND hdlg ) ;
	BOOL bRunningWin95();
/*
**  Purpose:
...
BOOL FAR PASCAL FCheckDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
    ...
    case WM_COMMAND:
        switch (wParam)
			...
			UINT bFlag;
        case IDC_B1:
			bFlag =IsDlgButtonChecked(hdlg, wParam); 
            CheckDlgButton(hdlg, wParam,
                    !bFlag);
			EnableWindow(GetDlgItem(hdlg,IDC_PRINTERS),!bFlag);
			break;
        ...
        case IDC_H:
            //HdlgShowHelp();
            if (!WinHelp(hdlg, "InstOpt.HLP", HELP_CONTENTS, (DWORD)(LPSTR)"")) {
                MessageBox (GetFocus(),
                     "Unable to activate help",
                     "Setup Message", MB_SYSTEMMODAL|MB_OK|MB_ICONHAND);
                }
                                        break;

            return(fTrue);
		case IDC_PRINTERS:
            if (!FSetSymbolValue("DLGEVENT", "BTN"))
                {
    ...
    return(fFalse);
// -------------------------------------------------
// Dummy calls, to force inclusion of these routines
// -------------------------------------------------
SetUnInstallText(0);
GetModelessDialog(0);
}
...
BOOL FAR PASCAL FCustInstDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
    ...
    case WM_INITDIALOG:
		...
		if (bRunningWin95())
		{
			EnableWindow(GetDlgItem(hdlg, IDC_B0 + 3), 0);
			EnableWindow(GetDlgItem(hdlg, IDC_B0 + 4), 0);
			EnableWindow(GetDlgItem(hdlg, IDC_SP1), 0);
		}
		bCenterWindow(hdlg);
        return(fTrue);
...
/*
**  Purpose:
**      Edit Dialog procedure for templates with two Edit controls.
**      (Limits the input string length to cbFullPathMax characters.)
**
**  Controls Recognized:
...
**  Initialization Symbols:
**      "EditTextIn" - initial text for IDC_EDIT edit control.
**		"Edit2TextIn" - initial text for the IDC_EDIT2 edit control.
**      "EditFocus"  - position of intial focus for text string:
**              "END" (default), "ALL", or "START"
**
**  Termination Symbols:
**      "EditTextOut" - text in the IDC_EDIT edit control upon termination.
**		"Edit2TextOut" - text in the IDC_EDIT2 edit control upon termination.
**      "DLGEVENT"    - one of the following, depending on event:
**              event                value
**              ----------           ----------
**              IDC_B                "BACK"
...
**
*****************************************************************************/
BOOL FAR PASCAL F2EditDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
    static WORD wSelStart = 0;
    static WORD wSelEnd   = 0;
    char  rgchText[cbFullPathMax + 1];
    ...
    case WM_INITDIALOG:
		...
		// Now do the second edit field's initialization
		cb = CbGetSymbolValue("Edit2TextIn", szSymBuf, cbFullPathMax + 1);
        Assert(cb < cbFullPathMax + 1);
    ...
    case WM_COMMAND:
        ...
        case IDCANCEL:
            ...
            SendDlgItemMessage(hdlg, IDC_EDIT2, (WORD)WM_GETTEXT,
              	cbFullPathMax + 1, (LONG)((LPSTR)rgchText));
            if (!FSetSymbolValue("Edit2TextOut", rgchText))
            {
                DestroyWindow(GetParent(hdlg));
                return(fTrue);
            }
...
BOOL FAR PASCAL FUnInstallDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
    switch (wMsg)
        {
    case WM_INITDIALOG:
...
void SetUnInstallText(char *pText)
{
  SetDlgItemText(hGlobalDialog,101,pText);
}
...
**
*****************************************************************************/
BOOL FAR PASCAL FPrinterDlgProc(HWND hdlg, WORD wMsg, WORD wParam, LONG lParam)
{
    char rgchNum[10];
    int  iButtonChecked;
    char szSymBuf[cbSymBuf];
    ...
    case WM_INITDIALOG:
		...
		if (iButtonChecked) 
 			for (i = 0; i < 4 ; ++i)
                 EnableWindow(GetDlgItem(hdlg, IDC_RADIO5 + i),1);
  
       SendDlgItemMessage(hdlg, IDC_RADIO1 + iButtonChecked, BM_SETCHECK,1,0L);
        cb = CbGetSymbolValue("RadioDefault1", szSymBuf, cbSymBuf);
        Assert(cb < cbSymBuf);
        if (*szSymBuf != '\0')
            {
            iButtonChecked = AsciiToInt((LPSTR)szSymBuf);
            ...
            iButtonChecked = 0;
         SendDlgItemMessage(hdlg, IDC_RADIO3 + iButtonChecked, BM_SETCHECK,1,0L);
    ...
    case WM_COMMAND:
        ...
        case IDC_RADIO1:
        case IDC_RADIO2:
            CheckRadioButton(hdlg, IDC_RADIO1, IDC_RADIO2, wParam);
			j = (wParam == IDC_RADIO1) ? 0 : 1;
			for(i = 0; i < 4;i++)
                 EnableWindow(GetDlgItem(hdlg, IDC_RADIO3 + i),j);
        ...
        case IDC_RADIO6:
            ...
            CheckRadioButton(hdlg, IDC_RADIO3, IDC_RADIO6, wParam);
			break;
        ...
        case IDCANCEL:
            ...
            iButtonChecked = 0;
            for (i = 0; i <= 1; i++)
                if (SendDlgItemMessage(hdlg, IDC_RADIO1 + i, BM_GETCHECK, 0, 0L))
                    {
                    iButtonChecked = i;
                    break;
                    }
            ...
            for (i = 0; i <= 4; i++)
                if (SendDlgItemMessage(hdlg, IDC_RADIO3 + i, BM_GETCHECK, 0, 0L))
                    {
                    iButtonChecked = i;
                    break;
                    }
            ...
            IntToAscii((int)iButtonChecked, (LPSTR)rgchNum);
            if (!FSetSymbolValue("ButtonChecked1", rgchNum))
                {
                DestroyWindow(GetParent(hdlg));
                return(fTrue);
                }
